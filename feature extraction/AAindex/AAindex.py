...
fastas = readFasta.readFasta(r"Kbhb_15_train.txt")
kw=  {'path': r"AAINDEX",'train':r"S_train.txt",'order':'ARNDCQEGHILKMFPSTWYVX'}
result=AAINDEX(fastas,**kw)

data=np.matrix(result[1:])
data_AAINDEX=pd.DataFrame(data=data[:,1:])
data_AAINDEX.to_csv('AAindex_Kbhb_15_train.csv')